...
export async function mapIdentityConversation(conversation: Conversation<MyContext, MyContext>, ctx: MyContext): Promise<void> {
  return runCancellable(async () => {
    ...
    await ctx.reply(
      "Выберите сотрудника, укажите его id (/cancel — отменить):\n" +
        employees
          .map(
            (e) =>
    ...
    if (jiraCreds) {
      ...
      const jiraMsg = await waitFor(conversation, ctx, "message:text");
      ...
      if (jiraRaw !== "-") {
        if (/^[0-9a-f]{24}$/i.test(jiraRaw) || jiraRaw.includes(":")) {
          ...
          if (candidates.length === 0) {
            ...
            const pickMsg = await waitFor(conversation, ctx, "message:text");
